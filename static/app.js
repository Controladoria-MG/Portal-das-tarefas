...
const UNIDADES = [
  ...
  { cod: 'MG_EXPRESS', nome: 'Mg Express',     arquivo: 'dados/base/mg_express.xlsx' },
...
const TODAS_CHAVES_TAB = ['cliente', 'grupo', 'tarefa', 'coordenador', 'responsavel', 'baixadopor', 'competencia', 'vencimento', 'previsao', 'databaixa', 'prioridade', 'comentario', 'tipopendencia'];

function comentarioStatus(r) {
  return String(r.Comentario || '').trim() ? 'Com comentário' : 'Sem comentário';
}
...
function colunasTab() {
  const colPrevisaoOuBaixa = estado.abaAtiva === 'Baixado'
    ...
    : { key: 'previsao',  label: 'Previsão',      valor: r => fmtData(r.DataPrevisaoConclusao), ordem: r => ordemData(r.DataPrevisaoConclusao) };
  ...
  const cols = [
    ...
    colResponsavel,
    { key: 'competencia', label: 'Competência', valor: r => fmtCompetencia(r.Competencia), ordem: r => ordemData(r.Competencia) },
    { key: 'vencimento',  label: 'Vencimento',  valor: r => fmtData(r.DataVencimento), ordem: r => ordemData(r.DataVencimento) },
    colPrevisaoOuBaixa,
    { key: 'prioridade',  label: 'Prioridade',  valor: r => String(r.Prioridade || '') },
    { key: 'comentario',  label: 'Comentário',  valor: comentarioStatus },
  ];

  // Coluna Operação/Gerência só existe na base SP (única com departamentos
  // de gerência identificáveis pelo nome no momento).
  if (estado.unidade && estado.unidade.cod === 'SP') {
    cols.push({ key: 'tipopendencia', label: 'Tipo de Pendência', valor: r => String(r.TipoPendencia || '') });
  }
...
// Converte para timestamp ordenável. Algumas colunas chegam da planilha como
// texto ("dd/mm/aaaa" na Previsão, "mm/aaaa" na Competência) em vez de Date, o
// que fazia o filtro dessas colunas listar as datas fora de ordem.
function ordemData(v) {
  if (v instanceof Date) return v.getTime();
  if (typeof v === 'string') {
    const dma = v.trim().match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
    if (dma) return new Date(+dma[3], +dma[2] - 1, +dma[1]).getTime();
    const ma = v.trim().match(/^(\d{1,2})\/(\d{4})$/);
    if (ma) return new Date(+ma[2], +ma[1] - 1, 1).getTime();
  }
  return 0;
}
...
function contarStatus(rows) {
  let emAtraso = 0, emAberto = 0, vencendoHoje = 0, baixadas = 0;
  for (const r of rows) {
    if      (r.Status === 'Em Atraso')     emAtraso++;
    else if (r.Status === 'Em Aberto')     emAberto++;
    else if (r.Status === 'Vencendo Hoje') vencendoHoje++;
    else if (r.Status === 'Baixado')       baixadas++;
  }
  return { total: rows.length, emAtraso, emAberto, vencendoHoje, baixadas };
}
...
function processarLinhas(cod, rows) {
  ...
  for (const r of rows) {
    if (r.Status === 'Em Aberto' && r.DataVencimento instanceof Date &&
        normData(r.DataVencimento).getTime() === hoje.getTime()) {
...
function _carregarArquivo(cod, arquivo) {
  PROMESSAS[cod] = fetch(arquivo)
    ...
    .then(buf => {
      ...
      DADOS_UNIDADE[cod] = processarLinhas(cod, XLSX.utils.sheet_to_json(ws, { raw: true }));
    })
    .catch(err => {
...
// Conta, dentro de um status específico, quantas linhas são Operação x Gerência
// (só tem sentido na base SP — outras unidades não têm TipoPendencia).
function contarTipoPendencia(rows, status) {
  let operacao = 0, gerencia = 0;
  for (const r of rows) {
    if (r.Status !== status) continue;
    if      (r.TipoPendencia === 'Gerência') gerencia++;
    else if (r.TipoPendencia === 'Operação') operacao++;
  }
  return { operacao, gerencia };
}

// ── Placares ─────────────────────────────────────────────
// opts.clicavel: se os cards levam a algum lugar ao clicar.
// opts.modo: 'aba'     → já está na tela 3, só troca de aba (dep. ou unidade inteira);
//            'unidade' → está na tela 2, o clique precisa abrir a tela 3 no modo
//                        "unidade inteira" já na aba certa.
function renderPlacares(containerId, rows, opts = {}) {
  const { clicavel = false, modo = 'aba' } = opts;
  const c = contarStatus(rows);
  const pct = v => c.total ? `${Math.round((v / c.total) * 100)}% do total` : '0% do total';
  // Base SP: processarLinhas() grava TipoPendencia em toda linha; nas demais
  // unidades essa chave nunca existe.
  const ehSP = rows.some(r => r.TipoPendencia !== undefined);

  const defs = [
    { classe: 'total',     valor: c.total,        label: 'Total de Tarefas', desc: 'da competência', status: null },
    { classe: 'atraso',    valor: c.emAtraso,      label: 'Em Atraso',       desc: pct(c.emAtraso),      status: 'Em Atraso', tipos: ehSP ? contarTipoPendencia(rows, 'Em Atraso') : null },
    { classe: 'vencehoje', valor: c.vencendoHoje,  label: 'Vencendo Hoje',   desc: pct(c.vencendoHoje),  status: 'Vencendo Hoje' },
    { classe: 'aberto',    valor: c.emAberto,      label: 'Em Aberto',      desc: pct(c.emAberto),      status: 'Em Aberto' },
    { classe: 'baixado',   valor: c.baixadas,      label: 'Baixadas',       desc: pct(c.baixadas),      status: null },
  ];

  const chamadaStatus = status => modo === 'unidade' ? `mostrarTela3(null, '${status}')` : `irParaAba('${status}')`;
  const chamadaTipo    = (status, tipo) => modo === 'unidade'
    ? `mostrarTela3ComTipo('${status}', '${tipo}')`
    : `irParaAbaTipo('${status}', '${tipo}')`;

  document.getElementById(containerId).innerHTML = defs.map(p => {
    if (p.tipos) {
      const attrOp  = clicavel ? `onclick="${chamadaTipo(p.status, 'Operação')}"` : '';
      const attrGer = clicavel ? `onclick="${chamadaTipo(p.status, 'Gerência')}"` : '';
      return `
      <div class="placar ${p.classe} placar-dividido">
        <div class="placar-label">${p.label}</div>
        <div class="placar-split">
          <div class="placar-metade${clicavel ? ' placar-metade-clicavel' : ''}" ${attrOp}>
            <div class="placar-metade-valor">${p.tipos.operacao.toLocaleString('pt-BR')}</div>
            <div class="placar-metade-label">Operação</div>
          </div>
          <div class="placar-divisor"></div>
          <div class="placar-metade${clicavel ? ' placar-metade-clicavel' : ''}" ${attrGer}>
            <div class="placar-metade-valor">${p.tipos.gerencia.toLocaleString('pt-BR')}</div>
            <div class="placar-metade-label">Gerência</div>
          </div>
        </div>
    ...
    const podeClicar = clicavel && p.status;
    return `
    <div class="placar ${p.classe}${podeClicar ? ' placar-clicavel' : ''}" ${podeClicar ? `onclick="${chamadaStatus(p.status)}"` : ''}>
      <div class="placar-label">${p.label}</div>
...
// Leva o clique num placar (dentro da tela 3) direto para a aba correspondente
// na tabela de tarefas, limpando qualquer filtro de Tipo de Pendência deixado
// por irParaAbaTipo.
function irParaAba(status) {
  if (filtrosTab.excluidos.tipopendencia) filtrosTab.excluidos.tipopendencia = new Set();
  const tab = document.querySelector(`.tab[data-status="${status}"]`);
  if (tab) mudarAba(tab);
}

// Igual a irParaAba, mas já deixa a tabela filtrada só por Operação ou só por
// Gerência — usado pelas metades do card "Em Atraso" dividido (dentro da tela 3).
function irParaAbaTipo(status, tipo) {
  filtrosTab.excluidos.tipopendencia = new Set(['Operação', 'Gerência'].filter(v => v !== tipo));
  const tab = document.querySelector(`.tab[data-status="${status}"]`);
  if (tab) mudarAba(tab);
}
...
async function mostrarTela2(cod, nome) {
  ...
  renderPlacares('placares-tela2', estado.unidadeRows, { clicavel: true, modo: 'unidade' });
    ...
    return `
      <div class="dep-card" onclick="mostrarTela3('${esc(dep)}')">
        ...
        <div class="dep-linhas">
          ...
          <div class="dep-linha">
            <span class="dep-dot vencehoje"></span>
            <span class="dep-linha-texto">Vencendo Hoje</span>
            <span class="dep-linha-valor">${c.vencendoHoje.toLocaleString('pt-BR')}</span>
          </div>
          <div class="dep-linha">
...
async function gerarPDF() {
  ...
  const subHead = `${estado.unidade.nome}  —  ${estado.departamento || 'Todas as tarefas'}`;
  const filtrosAtivos = [
  ...
  const depSlug = (estado.departamento || 'geral').replace(/[^a-z0-9]/gi, '_');
  doc.save(`relatorio_baixas_${depSlug}_${ano}-${String(mes+1).padStart(2,'0')}.pdf`);
}

// ── Tela 3 ───────────────────────────────────────────────
// `dep` null/omitido = visão geral da unidade inteira (todos os departamentos
// juntos), acionada pelos placares da tela 2. `statusInicial` define qual aba
// já abre selecionada (default 'Em Atraso', como nos cards de departamento).
function mostrarTela3(dep, statusInicial = 'Em Atraso') {
  estado.departamento = dep || null;
  estado.depRows      = dep ? estado.unidadeRows.filter(r => r.Departamento === dep) : estado.unidadeRows;
  estado.abaAtiva      = statusInicial;
  ...
  setBreadcrumb(['Painel de controle', estado.unidade.nome, dep || 'Todas as tarefas']);
  renderPlacares('placares-tela3', estado.depRows, { clicavel: true, modo: 'aba' });
  renderRelatorios(estado.depRows);

  document.querySelectorAll('.tab').forEach(t => t.classList.remove('ativo'));
  const tabInicial = document.querySelector(`.tab[data-status="${statusInicial}"]`);
  if (tabInicial) tabInicial.classList.add('ativo');

  renderTabela(statusInicial);
  trocarTela('tela-3');
}

// Mesma navegação acima, mas já deixando a tabela filtrada só por Operação ou
// só por Gerência — usado pelas metades do card "Em Atraso" dividido na tela 2.
function mostrarTela3ComTipo(status, tipo) {
  mostrarTela3(null, status);
  filtrosTab.excluidos.tipopendencia = new Set(['Operação', 'Gerência'].filter(v => v !== tipo));
...
function abrirPainelFiltro(key) {
  ...
  painel.innerHTML = `
    <input type="text" class="filtro-col-busca" placeholder="Pesquisar..." oninput="filtrarOpcoesColuna(this.value)" onkeydown="if(event.key==='Enter'){event.preventDefault();aplicarBuscaColuna();}" />
...
// Enter na caixa de pesquisa do filtro: seleciona apenas as opções que casam
// com o texto digitado (as visíveis) e aplica o filtro na hora — estilo Excel.
function aplicarBuscaColuna() {
  if (!filtroColunaAberta) return;
  const opcoes = [...document.querySelectorAll(`#lista-${filtroColunaAberta} .filtro-col-opcao`)];
  ...
  opcoes.forEach(el => {
    ...
    chk.checked = visivel;
    if (visivel) draftExcluidos.delete(chk.value);
...
function renderTabela(status) {
  ...
  const filtradas = filtrarRowsTab(estado.depRows.filter(r => r.Status === status));
  // Na aba "Baixadas" ordena da baixa mais recente para a mais antiga; nas
  // demais, mantém a ordenação por vencimento/cliente.
  const rows = ehBaixado
    ? [...filtradas].sort((a, b) =>
        (b.DataBaixa instanceof Date ? b.DataBaixa.getTime() : 0) -
        (a.DataBaixa instanceof Date ? a.DataBaixa.getTime() : 0))
    : sortRows(filtradas);
  ...
  contador.innerHTML = totalFiltrado
    ? `<span>${totalFiltrado.toLocaleString('pt-BR')}</span> Tarefa${totalFiltrado !== 1 ? 's' : ''}`
    : '';
  ...
  corpo.innerHTML = visiveis.map((r, idx) => {
